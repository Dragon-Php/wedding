                        ...
                        <table class="table table-striped table-hover table-bordered" id="sample_editable_1">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td> {{$i}} </td>
                                    <td> {{$rowData->title}} </td>
                                    <td> {{$rowData->auther}} </td>
                                    <td> {{$rowData->short_description}} </td>
                                    <td> {{Html::image($rowData->image, '', ['width'=>'100'])}} </td>
                                    
                                    <td>
                                        <a class="btn btn-sm btn-primary" title="Edit" href="{{ url('Admin-EditBlog/'.$rowData->id)}}"> <i class="fa fa-pencil"></i> </a>
                                        <a class="btn btn-sm btn-danger" title="Delete" href="{{ url('Admin-DeleteBlog/'.$rowData->id)}}"> <i class="fa fa-trash"></i> </a>
                                    </td>
                                </tr>
                                @php $i++; @endphp
                                @endforeach
